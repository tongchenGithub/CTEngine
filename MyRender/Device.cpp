...
#define MODEL_DRAW_BOX 1
#define MODEL_DRAW_MESH 2
#define MODEL_DRAW_TETRAHEDRON 4
...
void Device::setDrawModel(int s)
{
	drawmodel = s;
}
...
void Device::ChangeDrawModel()
{
	drawmodel += 1;
}


void Device::ChangeBmp()
{
	if (drawBmp == 1)
	{
		drawBmp = 0;
...
int Device::GetRealModel()
{
	int s = drawmodel % 3;
	if (s == 0) {
		return MODEL_DRAW_MESH;
	}
	else if (s == 1) {
		return MODEL_DRAW_BOX;
	}
	else if (s == 2) {
		return MODEL_DRAW_TETRAHEDRON;
	}
	return 0;
}
